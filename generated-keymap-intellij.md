|ID|Name|Description|Shortcut-1|Shortcut-2|
| --- | --- | --- | ---------- |--------|
|$Copy|Copy|Copy to clipboard|`control c`|`control insert`|
|$Cut|Cut|Cut to clipboard|`control x`|`shift delete`|
|$Delete|Delete|Delete selected item|`delete`|
|$Paste|Paste|Paste from clipboard|`control v`|`shift insert`|
|$Redo|Redo||`control shift z`|`alt shift back_space`|
|$SelectAll|Select All|Select all|`control a`|
|$Undo|Undo||`control z`|`alt back_space`|
|AIAssistant.Chat.AIPopupChat|||`control shift alt back_slash`|
|ActivateBookmarksToolWindow|||`alt 2`|
|ActivateCommitToolWindow|||`alt 0`|
|ActivateDebugToolWindow|Debug||`alt 5`|
|ActivateFavoritesToolWindow|||
|ActivateFindToolWindow|Find||`alt 3`|
...
|ActivateRunToolWindow|Run||`alt 4`|
|ActivateServicesToolWindow|||`alt 8`|
|ActivateStructureToolWindow|||`alt 7`|
|ActivateTerminalToolWindow|||`alt f12`|
|ActivateUnitTestsToolWindow|||`alt shift 8`|
|ActivateVersionControlToolWindow|||`alt 9`|
|AutoIndentLines|Auto-Indent Lines|Indent current line or selected block according to the code style settings|`control alt i`|
|Back|Back|Undo last navigation operation|`control alt left`|
|BraceOrQuoteOut|Jump Outside Current Bracket/Quote||`tab`|
|CallHierarchy|Call Hierarchy|Browse call hierarchy for the selected method|`control alt h`|
|ChangeSignature|Change Signature...|Change signature of the selected method or class and correct all references|`control f6`|
|ChangeTypeSignature|Type Migration...|Change type of the return type of the method, field, parameter, variable or class type arguments and correct all references|`control shift f6`|
|ChangesView.GroupBy.Directory|Directory||`control alt p`|
|ChangesView.GroupBy.Module|Module||`control alt m`|
|ChangesView.Move|Move to Another Changelist...|Move selected changes to another changelist|`alt shift m`|
|ChangesView.Rename|Edit Changelist...|Edit name and description of selected changelist|`f2`|`shift f6`|
|ChangesView.Revert|Rollback...||`control alt z`|
|ChangesView.SetDefault|Set Active Changelist|Set changelist to which new changes are placed by default|`control space`|
|ChooseDebugConfiguration|Debug...|Choose and debug configuration|`alt shift f9`|
|ChooseRunConfiguration|Run...|Choose and run configuration|`alt shift f10`|
...
|EditorFocusGutter|Focus Gutter (accessibility)||`alt shift 6,f`|
...
|FileChooser.GotoHome|Home Directory|Go to the home directory|`control 1`|
...
|Move|Move...|Move the selected class, method, package or static member to another package or class and correct all references|`f6`|
...
|RecentLocations|Recent Locations|Show list of recently viewed/changed locations|`control shift e`|
...
|ShelvedChanges.Rename|Rename...|Rename shelved changelist|`f2`|`shift f6`|
...
|SwitcherIterateItems|Iterate Recent Files||`control e`|
...
|UsageView.Include|Include|Include this usage(s) into processing|`insert`|
...
| AnnotateStackTraceAction.show.files.modification.info | Show files modification info |  | | |
...
| ChangeSortingAction | Sort by Name |  | | |
...
| ChooseRuntime | Choose Boot Java Runtime for the IDE... |  | | |
...
| CommittedChanges.Filter | Filter | Change filtering criteria | | |
...
| CopyUrl | Copy URL | Copy the URL under caret to clipboard | | |
...
| Debugger.MuteRenderers | Mute Renderers |  | | |
...
| Diff.ComparePartial.Left.Right | Left and Right |  | | |
...
| Documentation.KeepTab | Keep This Documentation | Turns off all updates in the current tab, and makes the next action invocation open a fresh tab | | |
...
| EditorBackwardParagraphWithSelection | Move Caret Backward a Paragraph with Selection |  | | |
...
| EditorNextWordInDifferentHumpsMode | Move Caret to Next Word in Different "CamelHumps" Mode |  | | |
...
| EmmetNextEditPoint | Next Emmet Edit Point | Go to next Emmet edit point | | |
...
| ExternalSystem.RunTask | Run Task | Execute selected tasks | | |
...
| ForceIndexRebuild | Reindex Everything |  | | |
...
| GotoNextBookmark | Next Line Bookmark |  | | |
...
| Graph.Layout.ChannelLayouter | Channel |  | | |
...
| Help.KeymapReference | Keyboard Shortcuts PDF | Open PDF file with the default keymap reference card. | | |
...
| JTabbedPaneDemo | JTabbedPane Demo |  | | |
...
| MakeModule | Build Module | Compile all modified and dependent files in the module | | |
...
| MoveTabRight | Split and Move Right | Split editor area into 2 tab groups and move the current tab to the right group and close the left editor | | |
...
| OpenMouseWheelSmoothScrollSettings | Smooth Scroll Options |  | | |
...
| ProblemsView.SortFoldersFirst | Folders Always on Top | Sort files and folders separately | | |
...
| ProjectView.SortByName | Sort by Name | Sort files by their name | | |
...
| RestoreFontPreviewTextAction | Restore Preview Text |  | | |
...
| ScrollPane-scrollLeft | Scroll Page Left | Scrolls the surrounding scrollable area left by one page. | | |
...
| ShowBlankDiffWindow | Open Blank Diff Window | Open two editors side-by-side and compare their content | | |
...
| ShowSearchEverywhereItemInfo | Show Search Everywhere Item Info |  | | |
...
| SliceBackward | Analyze Data Flow to Here... | Search for values which can be assigned to selected expression | | |
| SliceForward.MainMenu | Data Flow from Here... |  | | |
| SliceForward | Analyze Data Flow from Here... | Search for values escaped from this location | | |
| SmartSelect | Extend Selection |  | | |
| SmartUnSelect | Shrink Selection |  | | |
| SpeedSearch.GotItTooltip | Type any symbol to open the search box. Pressing {0} is not necessary. |  | | |
| SpeedSearch | Speed Search |  | | |
| SplitChooser.Exit | Exit chooser |  | | |
| SplitChooser.SplitDown | Use down split or switch down |  | | |
| SplitChooser.SplitLeft | Use left split or switch left |  | | |
| SplitChooser.SplitRight | Use right split or switch right |  | | |
| SplitChooser.SplitTop | Use top split or switch up |  | | |
| SplitHorizontally | Split Down | Split editor area into 2 tab groups and move the current tab to the bottom group | | |
| SplitVertically | Split Right | Split editor area into 2 tab groups and move the current tab to the right group | | |
| Start.Use.Vcs.Vcs.Toolbar.Widget | Other Services... |  | | |
| Start.Use.Vcs | Enable Version Control Integration\u2026 |  | | |
| StartStopMacroRecording | Start/Stop Macro Recording | Start/Stop recording of a new macro | | |
| StaticIconFields | List Static Icon Fields Initializers |  | | |
| StatusTextModeAction | Status Text |  | | |
| StepOverInstruction | Step Over Instruction | Perform Step Over on instruction level | | |
| StopWithDropDown | Stop with Options |  | | |
| StoreDefaultLayout | Save Changes in Current Layout | Save changes in the currently selected layout | | |
| StoreNewLayout | Save Current Layout as New... | Save the current layout under a new name | | |
| StretchSplitToBottom | Stretch Editor to Bottom | Stretch editor split to bottom | | |
| StretchSplitToLeft | Stretch Editor to Left | Stretch editor split to left | | |
| StretchSplitToRight | Stretch Editor to Right | Stretch editor split to right | | |
| StretchSplitToTop | Stretch Editor to Top | Stretch editor split to top | | |
| SurroundWithEmmet | Surround with Emmet | Surround selected code with Emmet abbreviation | | |
| SwapSidesInDiffWindow | Swap Sides |  | | |
| SwapThreeWayColorModeInDiffWindow | Change Three-Side Viewer Mode |  | | |
| SwitchFileBasedIndexStorageAction | Switch File-Based Index Storage |  | | |
| SwitchIdeScaleAction | Zoom IDE |  | | |
| SwitcherBackward | Iterate Switcher Backward |  | | |
| SwitcherForward | Iterate Switcher Forward |  | | |
| SwitcherNextProblem | Select Next Problem File |  | | |
| SwitcherPreviousProblem | Select Previous Problem File |  | | |
| SynchronizeCurrentFile | Reload from Disk |  | | |
| TabList | Show Hidden Tabs | Show dropdown with the list of tabs that don't fit on the screen in single-row tab layout | | |
| Tabbed.WelcomeScreen.OpenProject | Open |  | | |
| Table-scrollDownChangeSelection | Scroll Page Down and Select Row | Scrolls the surrounding scrollable area down by one page and selects the corresponding row. | | |
| Table-scrollDownExtendSelection | Scroll Page Down and Extend Selection | Scrolls the surrounding scrollable area down by one page and adds corresponding rows to selection. | | |
| Table-scrollUpChangeSelection | Scroll Page Up and Select Row | Scrolls the surrounding scrollable area up by one page and selects the corresponding row. | | |
| Table-scrollUpExtendSelection | Scroll Page Up and Extend Selection | Scrolls the surrounding scrollable area up by one page and adds corresponding rows to selection. | | |
| Table-selectFirstRow | Select First Row | Selects the first row. | | |
| Table-selectFirstRowExtendSelection | Extend Selection to First Row | Scrolls to the first row and adds corresponding rows to selection. | | |
| Table-selectLastRow | Select Last Row | Selects the last row. | | |
| Table-selectLastRowExtendSelection | Extend Selection to Last Row | Scrolls to the last row and adds corresponding rows to selection. | | |
| Table-selectNextColumn | Select Next Column | Selects the next column. | | |
| Table-selectNextColumnExtendSelection | Extend Selection to Next Column | Adds the next column to selection. | | |
| Table-selectNextRow | Select Next Row | Selects the next row. | | |
| Table-selectNextRowExtendSelection | Extend Selection with Next Row | Adds the next row to selection. | | |
| Table-selectPreviousColumn | Select Previous Column | Selects the previous column. | | |
| Table-selectPreviousColumnExtendSelection | Extend Selection to Previous Column | Adds the previous column to selection. | | |
| Table-selectPreviousRow | Select Previous Row | Selects the previous row. | | |
| Table-selectPreviousRowExtendSelection | Extend Selection with Previous Row | Adds the previous row to selection. | | |
| Table-startEditing | Edit Table Cell |  | | |
| TechnicalSupport | Contact Support... | Contact technical support on JetBrains website | | |
| TemplateProjectStructure.MainMenu | Structure... |  | | |
| TerminalDecreaseFontSize | Decrease Font Size |  | | |
| TerminalIncreaseFontSize | Increase Font Size |  | | |
| TerminalResetFontSize | Reset Font Size |  | | |
| TestCoroutineProgress | Test Coroutine Progress |  | | |
| TestDndAction | Test Dnd |  | | |
| TestGestureAction | Test Gesture Action |  | | |
| TestIconMappings | Test Icon Mappings |  | | |
| TestMessageBoxAction | Show Test Dialog |  | | |
| TestParseEventsSchemeAction | Test Parsing Events Scheme... |  | | |
| TestProgressWindow | Test Progress Window |  | | |
| TestRubberDuckDebuggerAction | Test Rubber Duck Debugger |  | | |
| TestSearchContributors | SE Contributors Testing |  | | |
| TestTextFieldAction | Test Text Fields |  | | |
| TestTooltipsAction | Test Tooltips |  | | |
| TextComponent.ClearAction | Clear text | Clear text in text components | | |
| TextEditorWithPreview.SplitHorizontally | Horizontal layout |  | | |
| TextEditorWithPreview.SplitVertically | Vertical layout |  | | |
| TextGrayFilterConfig | &TextGrayFilter |  | | |
| ToggleBreakpointEnabled | Toggle Breakpoint Enabled | Enable/disable breakpoint at the current line | | |
| ToggleCompactMode | Compact Mode |  | | |
| ToggleCompletionHintsAction | Show parameter name hints on completion |  | | |
| ToggleContentUiTypeMode | Group Tabs | Toggle between tabbed/combo presentation of contents | | |
| ToggleDistractionFreeMode | Toggle Distraction Free mode | Toggle Distraction Free mode | | |
| ToggleDockMode | Docked Mode | Dock/undock active tool window | | |
| ToggleEssentialHighlighting | Essential Highlighting | Essential highlighting mode leaves only a limited set of code highlighters. All highlighters including inspections will be running on file saving actions | | |
| ToggleFieldBreakpoint.MainMenu | Field Watchpoint |  | | |
| ToggleFieldBreakpoint | Toggle Field Watchpoint | Toggle field watchpoint for the field at caret | | |
| ToggleFileBasedIndexes | Toggle File Based Indexes |  | | |
| ToggleFloatingMode | Floating Mode | Float/unfloat active tool window | | |
| ToggleFocusMode | Highlight Only Current Declaration |  | | |
| ToggleFullScreen | Toggle Full Screen mode | Toggle Full Screen mode | | |
| ToggleIncompleteMode | Enter Incomplete Dependencies Mode |  | | |
| ToggleInlineHintsAction | Toggle parameter name hints | Toggle parameter name hints | | |
| ToggleLineBreakpoint.MainMenu | Line Breakpoint |  | | |
| ToggleMethodBreakpoint.MainMenu | Method Breakpoint |  | | |
| ToggleMethodBreakpoint | Toggle Method Breakpoint | Toggle method breakpoint for the method at caret | | |
| TogglePinnedMode | Pinned Mode | Pin/unpin active tool window | | |
| TogglePowerSave | Power Save Mode | Power save mode disables background code analysis and other background operations | | |
| TogglePresentationAssistantAction | Presentation Assistant |  | | |
| TogglePresentationMode | Toggle Presentation mode | Toggle Presentation mode | | |
| ToggleRenderedDocPresentationForAll | Render All Doc Comments | Switch to rendered view for all documentation comments | | |
| ToggleSideMode | Split Mode | Tool windows split mode on/off | | |
| ToggleTemporaryLineBreakpoint.MainMenu | Temporary Line Breakpoint |  | | |
| ToggleThreeSideInBlankDiffWindow | Toggle Three-Side Viewer |  | | |
| ToggleUnusedState | Signal IDE Not Used | Signal the IDE is either not used or used. | | |
| ToggleWindowedMode | Windowed Mode | Window/unwindow active tool window | | |
| ToggleZenMode | Toggle Zen mode | Toggle Distraction Free and Full Screen modes simultaneously | | |
| ToolWindowShowNamesAction | Show Tool Window Names |  | | |
| ToolbarDemo | Toolbar Demo |  | | |
| ToolwindowSwitcher | Toolwindow Switcher |  | | |
| TopStripeEnableAction | Use Single Tool Window Bar |  | | |
| TopStripePinButton | Pin |  | | |
| TopStripeUnPinButton | Unpin |  | | |
| TransferSettingsDemoAction | Test Transfer Settings |  | | |
| Tree-scrollDownChangeSelection | Scroll Page Down and Select Node | Scrolls the surrounding scrollable area down by one page and selects the corresponding node. | | |
| Tree-scrollDownExtendSelection | Scroll Page Down and Extend Selection | Scrolls the surrounding scrollable area down by one page and adds corresponding nodes to selection. | | |
| Tree-scrollUpChangeSelection | Scroll Page Up and Select Node | Scrolls the surrounding scrollable area up by one page and selects the corresponding node. | | |
| Tree-scrollUpExtendSelection | Scroll Page Up and Extend Selection | Scrolls the surrounding scrollable area up by one page and adds corresponding nodes to selection. | | |
| Tree-selectChild | Select Child Node | Expands collapsed node or selects the child node. | | |
| Tree-selectChildExtendSelection | Extend Selection to Child Node | Expands collapsed node or adds the child node to selection. | | |
| Tree-selectFirst | Select First Node | Selects the first node. | | |
| Tree-selectFirstExtendSelection | Extend Selection to First Node | Scrolls to the first node and adds corresponding nodes to selection. | | |
| Tree-selectLast | Select Last Node | Selects the last node. | | |
| Tree-selectLastExtendSelection | Extend Selection to Last Node | Scrolls to the last node and adds corresponding nodes to selection. | | |
| Tree-selectNext | Select Next Node | Selects the next node. | | |
| Tree-selectNextExtendSelection | Extend Selection with Next Node | Adds the next node to selection. | | |
| Tree-selectNextSibling | Select Next Sibling Node | Selects the next sibling node. | | |
| Tree-selectParent | Select Parent Node | Collapses expanded node or selects the parent node. | | |
| Tree-selectParentExtendSelection | Extend Selection to Parent Node | Scrolls to the parent node and adds corresponding nodes to selection. | | |
| Tree-selectParentNoCollapse | Select Parent Node | Selects the parent node without collapsing expanded node. | | |
| Tree-selectPrevious | Select Previous Node | Selects the previous node. | | |
| Tree-selectPreviousExtendSelection | Extend Selection with Previous Node | Adds the previous node to selection. | | |
| Tree-selectPreviousSibling | Select Previous Sibling Node | Selects the previous sibling node. | | |
| Tree-startEditing | Edit Tree Node |  | | |
| TreeExpandAll | Expand Tree |  | | |
| TriggerLowMemoryNotification | Trigger Low Memory Notification |  | | |
| TriggerProjectIndexableFilesFilterHealthCheckAction | Trigger Project Indexable Files Filter Health Check |  | | |
| TrustProject | Trust |  | | |
| TurnOnOffCachedValueProfilerAction | Turn on Cached Value Profiler |  | | |
| TurnRefsToSuper | Use Interface Where Possible... | Change usages of a class to those of its superclass or interface | | |
...
| TypeHierarchyBase.BaseOnThisType | Base on This Type |  | | |
| TypingLatencyReport | Typing Latency Report |  | | |
| UIDefaults | LaF Defaults |  | | |
| UISandboxAction | UI Sandbox |  | | |
| UiDebugger | UI Debugger... |  | | |
| UiDslShowcaseAction | UI DSL Showcase |  | | |
| UiInspector | UI &Inspector |  | | |
| UndockMode | Undock | Make tool window sliding and auto-hiding on focus lost | | |
| UnmarkRoot | Unmark |  | | |
| Unscramble.EditorPopup | Stack Trace or Thread Dump... |  | | |
| Unscramble | Analyze Stack Trace or Thread Dump... | Open console with the navigatable stack trace or a thread dump | | |
| Unsplit | Unsplit | Unsplit window | | |
| UnsplitAll | Unsplit All | Remove all splitters | | |
| UntrustProject | Do not trust |  | | |
| UpdateFiles | Update |  | | |
| UpdateRunningApplication | Update Running Application |  | | |
| UsageView.Exclude | Exclude | Exclude this usage(s) from processing | | |
| UsageView.Remove | Remove | Remove this usage(s) from processing | | |
| UsageView.Rerun | Rerun | Rerun search | | |
| UsageView.ShowRecentFindUsages | Recent Find Usages | Choose and re-run recent find usages | | |
| UsedIconsListing | List Used Icons |  | | |
| UserInvokedFullAnalysis | Create And Immediately Analyze Heap Dump |  | | |
| ValidationTest | Validation Dialog Test |  | | |
| Vcs.ApplySelectedChanges | Cherry-Pick Selected Changes |  | | |
| Vcs.CombinedDiff.CaretToNextBlock | Next File |  | | |
| Vcs.CombinedDiff.CaretToPrevBlock | Previous File |  | | |
| Vcs.CombinedDiff.ToggleCollapseAllBlock | Collapse All Files | Toggle collapse all files in Combined Diff | | |
| Vcs.CombinedDiff.ToggleCollapseBlock | Collapse File |  | | |
...
| Vcs.Diff.ShowCombinedDiff | Show Combined Diff | Show diff for all selected changes in one place | | |
| Vcs.Diff.ShowDiffInEditorTab | Show Diff in Editor Tab |  | | |
| Vcs.Diff.ShowDiffInNewWindow | Show Diff in Separate Window |  | | |
| Vcs.Diff.ToggleDiffAligningMode.popup@DiffToolbar | Align Changes Highlighting |  | | |
| Vcs.Diff.ToggleDiffAligningMode | Align Changes In Side-by-Side Diff | Turn on changes alignment for two-sided diff | | |
| Vcs.Diff.ToggleSearchInChanges | Search In Changes |  | | |
| Vcs.EditSource | Edit Source |  | | |
| Vcs.GetVersion | Get | Get version from repository | | |
| Vcs.IntegrateProject | Integrate Project | Integrate project | | |
| Vcs.Operations.Popup.Annotate | Annotated Line Operations |  | | |
| Vcs.Push.Force | Force Push |  | | |
| Vcs.Push | Push... |  | | |
| Vcs.RevertSelectedChanges | Revert Selected Changes |  | | |
| Vcs.RunCommitChecks | Run Commit Checks |  | | |
| Vcs.Show.Local.Changes | Show Local Changes |  | | |
| Vcs.Show.Shelf | Show Shelf |  | | |
| Vcs.ShowConsoleTab | Show VCS Console Tab |  | | |
| Vcs.ShowDiffChangedLines | Show Diff for Lines | Show diff for selected lines | | |
| Vcs.ShowDiffWithLocal.Before | Compare Before with Local | Compare version before selected revision with current version | | |
| Vcs.ShowDiffWithLocal | Compare with Local | Compare version in selected revision with current version | | |
| Vcs.ShowHistoryForBlock | Show History for Selection\u2026 | Show history for editor selection | | |
| Vcs.ShowHistoryForRevision | History Up to Here | Show history for file until the selected revision | | |
| Vcs.ShowTabbedFileHistory.for.file | Show History for {0} |  | | |
| Vcs.ShowTabbedFileHistory | Show History | Show history of file | | |
| Vcs.Toolbar.ShowMoreActions | Version Control | VCS Operations | | |
| Vcs.UpdateProject | Update Project |  | | |
| Vcs.VcsClone | Get from Version Control... |  | | |
| VcsShowCurrentChangeMarker | Change Under Caret | Show popup for currently selected change | | |
| VcsToolbarLabelAction | VCS Label |  | | |
| ViewImportPopups | Show Import Popups | Toggle display import popups in current file | | |
| ViewInplaceComments | File Details | Show/hide extra information in tree views | | |
| ViewMainMenu | Main Menu | Show/hide the main menu | | |
| ViewMembersInNavigationBar.NavBar | Show Members in Navigation Bar |  | | |
| ViewMembersInNavigationBar | Members in Navigation Bar | Show/hide declarations in the navigation bar | | |
| ViewNavigationBar.NavBar | Show Navigation Bar |  | | |
| ViewNavigationBar | Navigation Bar | Show/hide the navigation bar | | |
| ViewNewToolbarAction | Toolbar |  | | |
| ViewObsoleteNavBarAction | Navigation Bar |  | | |
| ViewObsoleteToolbarAction | Toolbar Classic |  | | |
| ViewOfflineInspection | View Offline Inspection Results... | Load offline inspection results | | |
| ViewStatusBar | Status Bar | Show/hide the status bar | | |
| ViewToolBar | Toolbar | Show/hide the main toolbar | | |
| ViewToolButtons | Tool Window Bars | Show/hide tool window buttons | | |
| VirtualFileInfo | Virtual File Info |  | | |
| WebBrowser | Show &Web Browser |  | | |
| WelcomeScreen.ChangeProjectIcon | Set Custom Project Icon... |  | | |
| WelcomeScreen.Configure.Export | Export Settings |  | | |
| WelcomeScreen.Configure.Import | Import Settings... |  | | |
| WelcomeScreen.CopyProjectPath | Copy Path |  | | |
| WelcomeScreen.CreateDirectoryProject | New Project | Create a new project. | | |
| WelcomeScreen.CreateNewProject | New Project | Start the \"New Project\" Wizard that will guide you through the steps necessary for creating a new project. | | |
| WelcomeScreen.DevelopPlugins | Plugin Development | Get started developing plugins. | | |
| WelcomeScreen.EditGroup | Edit... |  | | |
| WelcomeScreen.ManageLicense | Manage Licenses... |  | | |
| WelcomeScreen.NewGroup | New Project Group |  | | |
| WelcomeScreen.OpenDirectoryProject | Open | Start editing code in the specified file or directory. | | |
| WelcomeScreen.OpenProject | Open or Import | Open file or project. You can also open a project or edit a file by drag and drop onto Welcome screen. | | |
| WelcomeScreen.OpenSelected | Open Selected |  | | |
| WelcomeScreen.Plugins.Extensions | Extensions |  | | |
| WelcomeScreen.Plugins | Plugins | Manage installed plugins and download new ones from repository | | |
| WelcomeScreen.RemoveSelected | Remove from Recent Projects\u2026 |  | | |
| WhatsNewAction | What's New | Find out about the new features in this version of the IDE | | |
| WindowMode | Window | Move tool window to a separate frame | | |
| WriteActionUnderProgress | Test Write Action Under Progress |  | | |
| X11UiTestAction | X11 UI Test |  | | |
| XDebugger.CompareValueWithClipboard | Compare Value with Clipboard | Compare value of selected node with clipboard text | | |
| XDebugger.CopyName | Copy Name | Copy name of selected node to clipboard | | |
| XDebugger.CopyValue | Copy Value | Copy value of selected node to clipboard | | |
| XDebugger.CopyWatch | Duplicate Watch |  | | |
| XDebugger.EditWatch | Edit... |  | | |
| XDebugger.Inline | Show Variable Values in Editor | Show available debugger session values inside editor | | |
| XDebugger.Inspect | Inspect... |  | | |
| XDebugger.JumpToSource | Jump To Source | Open source of the selected item | | |
| XDebugger.MoveWatchDown | Move Watch Down |  | | |
| XDebugger.MoveWatchUp | Move Watch Up |  | | |
| XDebugger.MuteBreakpoints | Mute Breakpoints |  | | |
| XDebugger.PinToTop | Pin to Top |  | | |
| XDebugger.PreviewTab | Open Files in Preview Tab |  | | |
| XDebugger.RemoveAllWatches | Remove All Watches |  | | |
| XDebugger.RemoveWatch | Remove Watch |  | | |
| XDebugger.SeparateWatches | Separate Watches |  | | |
| XDebugger.Show.Breakpoints.Over.Line.Numbers | Breakpoints Over Line Numbers |  | | |
| XDebugger.ToggleEvaluateExpressionField | Show Evaluate Expression Field |  | | |
| XDebugger.ToggleSortValues | Sort Variables Alphabetically | Sort values alphabetically in debugger views | | |
| XDebugger.UnmuteOnStop | Unmute Breakpoints on Session Finish |  | | |
| XSD2Document | Generate XSD Schema from XML File... |  | | |
| XcodeBuildSwiftDocumentationAction | Build Documentation | Build documentation for all targets | | |
| XcodePreviewSwiftDocumentationAction | Preview Documentation | Build and preview documentation for all targets | | |
| ZoomCurrentWindow | Zoom | Zoom current window | | |
| compare.archives | Compare Archives |  | | |
| compare.files | Compare Files |  | | |
| compare | Compare |  | | |
| compare.with | Compare With... |  | | |
| editRunConfigurations | Edit Configurations... | Open 'Edit Run/Debug configurations' dialog | | |
| fontEditorPreview.ToggleBoldFont | Toggle Bold Font |  | | |
| main.toolbar.Filename | File Name Widget (when editor tabs are hidden or \u201CAlways show full path\u201D is enabled) |  | | |
| main.toolbar.Project | Project Widget |  | | |
| multilaunch.AddExecutableAction | Add Configuration or Task |  | | |
| multilaunch.CancelExecutableAction | Cancel |  | | |
| multilaunch.DebugMultiLaunchAction | Debug |  | | |
| multilaunch.DeleteExecutableAction | Delete |  | | |
| multilaunch.EditExecutableAction | Edit\u2026 |  | | |
| multilaunch.RunMultiLaunchAction | Run |  | | |
| multilaunch.StopMultiLaunchAction | Stop |  | | |
| openAssertEqualsDiff | View assertEquals Difference |  | | |
| performancePlugin.ShowMemoryDialogAction | Change Memory Settings | Show change memory dialog | | |
| plugins.autoupdate.repository.view | View Plugins Auto-Update Repository |  | | |
| redo | Redo {0} | Redo {0} | | |
| search.everywhere.preview.close | Close Preview | Close search everywhere preview | | |
| synonym.Vcs.ShowHistoryForBlock.class | Show History for Class\u2026 |  | | |
| synonym.Vcs.ShowHistoryForBlock.method | Show History for Method\u2026 |  | | |
| undo | Undo {0} | Undo {0} | | |
